  ...
  const handleQRCodeScan = async (qrCodeData) => {
    ...
    try {
      ...
      if (progress === '3') {
        ...
        console.log('Aktualisiere Fortschritt auf 3'); // Debugging-Ausgabe
        ...
        if (updatedProfile.progress === 3) {
          navigate('/piratenstory_03'); // Ändere dies zur nächsten Story-Seite
        } else {
  ...
  const fetchHint = async () => {
    try {
      const { data: hintData, error: hintError } = await supabase
        .from('hints')
        .select('content')
        .eq('story_id', 2) // Ändere dies entsprechend der aktuellen Geschichte
        .single();
