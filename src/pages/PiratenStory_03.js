  ...
  const handleQRCodeScan = async (qrCodeData) => {
    ...
    try {
      ...
      if (progress === '4') {
        ...
        console.log('Aktualisiere Fortschritt auf 4'); // Debugging-Ausgabe
  ...
  const fetchHint = async () => {
    try {
      const { data: hintData, error: hintError } = await supabase
        .from('hints')
        .select('content')
        .eq('story_id', 3) // Ändere dies entsprechend der aktuellen Geschichte
        .single();
